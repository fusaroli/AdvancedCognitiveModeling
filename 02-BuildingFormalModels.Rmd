---
title: "Advanced Cognitive Modeling - Practical Exercise 2"
author: "RF"
date: "2/4/2022"
...
p2 <- ggplot(d2, aes(trial, choice)) + geom_line() + theme_classic()
...
p3 <- ggplot(d1, aes(trial, cumulativerate)) + geom_line() + ylim(0,1) + theme_classic()
p4 <- ggplot(d2, aes(trial, cumulativerate)) + geom_line() + ylim(0,1) + theme_classic()
...
p5 <- ggplot(d, aes(trial, cumulativerate, color=rate, group=rate)) + geom_line() + ylim(0,1) + theme_classic()
p5
...
for (agent in seq(agents)){
  ...
  if (agent==1){df <- temp} else {df <- bind_rows(df, temp)}
}}
